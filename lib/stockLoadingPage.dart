...
class _StockLoadingState extends State<StockLoading> with SingleTickerProviderStateMixin{
                              ...
                              ElevatedButton(
                                ...
                                if (truckIdController.text.isEmpty || resourceIdController.text.isEmpty || projectIdController.text.isEmpty) {
                                  ...
                                  await createNewLoad({
                                    ...
                                    "Number06": capacityController.text.isEmpty?"0" : capacityController.text,
                                    "Number07": volumeController.text.isEmpty?"0" : volumeController.text,
                                    "Number08": heightController.text.isEmpty?"0" : heightController.text,
                                    "Number09": widthController.text.isEmpty?"0" : widthController.text,
                                    "Number10": lengthController.text.isEmpty?"0" : lengthController.text,
  ...
  Future<void> getBinsFromWarehouse (String warehouseCode) async {
    ...
    try {
      final response = await http.get(
          Uri.parse("https://77.92.189.102/IIPrecastVerticalDemo/api/v1/Erp.BO.WhseBinSvc/WhseBins?\$filter=WarehouseCode eq '$warehouseCode'"),
          headers: {
      HttpHeaders.authorizationHeader: basicAuth,
      HttpHeaders.contentTypeHeader: 'application/json',
      });
