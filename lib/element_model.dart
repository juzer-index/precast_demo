...
class ElementData {
  ...
  factory ElementData.fromJson(Map<String, dynamic> json) {
    return ElementData(
      ...
      erectionSeq: int.parse((json['Number06'] ?? '0')),
      erectionDate: json['Date01'] ?? '',
      UOM: json['ShortChar07'],
      selectedQty: int.parse((json['Number01'] ?? '0') ),
      weight: double.parse(json['Number03'] ?? '0.0'),
      area: double.parse(json['Number04'] ?? '0.0'),
      volume: double.parse(json['Number05'] ?? '0.0'),
      quantity: 1,
